...
const ProjectDetailsPage = ({ onGoBack }: { onGoBack: () => void }) => {
    ...
    const handleSourceCodeClick = () => setIsModalOpen(true);
    const handleModalConfirm = () => setIsModalOpen(false);
    const handleModalCancel = () => setIsModalOpen(false);

    return (
        <div className="p-4 sm:p-6 md:p-0 bg-white min-h-screen font-sans">
            <div className="flex items-center mb-8">
                <button onClick={onGoBack} className="mr-4 text-gray-600 hover:text-gray-900 transition-colors" aria-label="Go back">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-6 p-4 md:p-8 bg-gray-100 rounded-lg shadow-inner">
                ...
                <div className="flex flex-col items-center">
                    <button onClick={handleSourceCodeClick} className="bg-white p-6 rounded-lg shadow-md border border-gray-200 w-full flex flex-col items-center justify-center space-y-2 hover:bg-gray-50 transition-colors duration-200">
                        <svg className="w-16 h-16 text-yellow-500" fill="currentColor" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg">
                            <path d="M10 4H4c-1.11 0-2 .89-2 2v12c0 1.11.89 2 2 2h16c1.11 0 2-.89 2-2V8c0-1.11-.89-2-2-2h-8l-2-2z" />
                        </svg>
            ...
            <DownloadModal isOpen={isModalOpen} onCancel={handleModalCancel} onConfirm={handleModalConfirm} />
        </div>
    );
};
...
const SuperAdminSubmissionsPage = () => {
    ...
    const [startYear, setStartYear] = useState<number | undefined>(2020);
    ...
    const mockArchivedProjects: ArchivedProject[] = [
        { id: 1, title: 'Research and Capstone Project Electronic Repository', proponents: ['Proponent 1', 'Proponent 2'], adviser: 'Sample Name', date: 'March 26, 2024', },
        { id: 2, title: 'Research and Capstone Project Electronic Repository', proponents: ['Proponent A', 'Proponent B'], adviser: 'Another Sample', date: 'February 15, 2024', },
    ];
    ...
    const filteredProjects = mockProjects.filter(project => {
        ...
        const projectStartYear = parseInt(project.schoolYear.split('-')[0], 10);
        return startYear && endYear ? (projectStartYear >= startYear && projectStartYear <= endYear) : true;
    });

    const filteredArchivedProjects = mockArchivedProjects.filter(project =>
        project.title.toLowerCase().includes(searchTerm.toLowerCase()) ||
        project.proponents.join(' ').toLowerCase().includes(searchTerm.toLowerCase())
    );
                    ...
                    <div className="flex flex-col md:flex-row items-center justify-between gap-4 p-4 bg-white shadow-md rounded-b-md border-t-0 border-gray-200 mb-8">
                        <div className="relative w-full md:w-auto flex-grow">
                            ...
                            <div className="absolute inset-y-0 right-0 flex items-center pr-2">
                                <button onClick={handleSearch} className="p-1 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-gray-400 rounded-full hover:bg-gray-100" aria-label="Search">
                                    <SearchIcon />
                                </button>
                            </div>
                        </div>
                        <div className="flex flex-col sm:flex-row items-center gap-4">
                            ...
                            <div className="flex items-center relative gap-2">
                                <YearPicker year={startYear} setYear={handleStartYearChange} placeholder="Start year" fromYear={fromYear} toYear={toYear} />
                                <YearPicker year={endYear} setYear={setEndYear} placeholder="End year" fromYear={startYear || fromYear} toYear={toYear} disabled={!startYear} />
                                {(startYear || endYear) && (
                                    <Button
                                        variant="ghost"
            ...
            content = (
                <>
                    <div className="flex items-center mb-6">
                        <button onClick={showSubmissions} className="mr-4 text-gray-600 hover:text-gray-900 transition-colors" aria-label="Go back">
                            <svg xmlns="http://www.w3.org/2000/svg" className="h-8 w-8" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                                <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M10 19l-7-7m0 0l7-7m-7 7h18" />
                            </svg>
                        </button>
                    ...
                    <div className="relative mb-6">
                        <input type="text" placeholder="Search here" value={searchTerm} onChange={(e) => setSearchTerm(e.target.value)} className="w-full pl-4 pr-10 py-3 border border-gray-300 rounded-md shadow-sm focus:outline-none focus:ring-2 focus:ring-gray-400" />
                        <div className="absolute inset-y-0 right-0 flex items-center pr-3"> <SearchIcon /> </div>
                    </div>
