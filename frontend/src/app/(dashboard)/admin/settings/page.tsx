...
const AdminSettingsPage = () => {
  ...
  const [originalData, setOriginalData] = useState<Partial<AdminFormData>>({});
  ...
  const fetchProfile = useCallback(async () => {
    if (
      ...
      authStore.getUser()?.role.toLowerCase() !== "admin"
    ) {
      router.push("/login");
      return;
    }
...
const ViewProfile = ({ userData }: { userData: Partial<AdminFormData> }) => {
  const fullName = [
    userData.first_name,
